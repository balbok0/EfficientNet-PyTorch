...
test_dataset = ImageNet(DATA_ROOT, split="val", transform=input_transform, target_transform=None)
...
evaluator = ImageNetEvaluator(model_name=model_name, paper_arxiv_id='1905.11946')
...
with torch.no_grad():
    for i, (input, target) in enumerate(test_loader):
        ...
        image_ids = [
            get_img_id(img[0])
            for img in test_loader.dataset.imgs[i * test_loader.batch_size : (i + 1) * test_loader.batch_size]
        ]
