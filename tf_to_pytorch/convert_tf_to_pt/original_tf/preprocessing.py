...
def _decode_and_random_crop(image_bytes, image_size):
    ...
    image = tf.cond(
        ...
        lambda: tf.image.resize_bicubic([image], [image_size, image_size])[0],  # pylint: disable=g-long-lambda
    )
...
def preprocess_for_train(
    image_bytes, use_bfloat16, image_size=IMAGE_SIZE, augment_name=None, randaug_num_layers=None, randaug_magnitude=None
):
...
def preprocess_image(
    ...
    if is_training:
        return preprocess_for_train(
            image_bytes, use_bfloat16, image_size, augment_name, randaug_num_layers, randaug_magnitude
        )
    else:
