...
flags.DEFINE_string('imagenet_eval_glob', None, 'Imagenet eval image glob, ' 'such as /imagenet/ILSVRC2012*.JPEG')
flags.DEFINE_string(
    'imagenet_eval_label',
    None,
    'Imagenet eval label file path, ' 'such as /imagenet/ILSVRC2012_validation_ground_truth.txt',
)
flags.DEFINE_string('ckpt_dir', '/tmp/ckpt/', 'Checkpoint folders')
flags.DEFINE_string('example_img', '/tmp/panda.jpg', 'Filepath for a single example image.')
flags.DEFINE_string('labels_map_file', '/tmp/labels_map.txt', 'Labels map from label id to its meaning.')
flags.DEFINE_integer('num_images', 5000, 'Number of images to eval. Use -1 to eval all images.')
FLAGS = flags.FLAGS
...
class EvalCkptDriver(object):
    ...
    def build_dataset(self, filenames, labels, is_training):
        ...
        def _parse_function(filename, label):
            ...
            image_decoded = preprocessing.preprocess_image(image_string, is_training, self.image_size)
...
def eval_imagenet(model_name, ckpt_dir, imagenet_eval_glob, imagenet_eval_label, num_images):
...
def main(unused_argv):
    ...
    if FLAGS.runmode == 'examples':
        ...
        eval_example_images(FLAGS.model_name, FLAGS.ckpt_dir, [FLAGS.example_img], FLAGS.labels_map_file)
    elif FLAGS.runmode == 'imagenet':
        # Run inference for imagenet.
        eval_imagenet(
            FLAGS.model_name, FLAGS.ckpt_dir, FLAGS.imagenet_eval_glob, FLAGS.imagenet_eval_label, FLAGS.num_images
        )
    else:
