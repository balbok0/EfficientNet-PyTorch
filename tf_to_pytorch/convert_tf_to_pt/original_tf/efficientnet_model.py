...
class MBConvBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, block_args, global_params):
        ...
        if self._block_args.condconv:
            self.conv_cls = functools.partial(condconv_layers.CondConv2D, num_experts=self._condconv_num_experts)
            self.depthwise_conv_cls = functools.partial(
                condconv_layers.DepthwiseCondConv2D, num_experts=self._condconv_num_experts
            )
    ...
    def _build(self):
        ...
        if self._block_args.condconv:
            ...
            self._avg_pooling = tf.keras.layers.GlobalAveragePooling2D(data_format=self._data_format)
            self._routing_fn = tf.layers.Dense(self._condconv_num_experts, activation=tf.nn.sigmoid)
    ...
    def _call_se(self, input_tensor):
        ...
        logging.info('Built Squeeze and Excitation with tensor shape: %s', (se_tensor.shape))
    ...
    def call(self, inputs, training=True, survival_prob=None):
        ...
        logging.info(
            'Block input depth: %s output depth: %s', self._block_args.input_filters, self._block_args.output_filters
        )
        ...
        if self._block_args.condconv:
            ...
            fused_conv_fn = functools.partial(self._fused_conv, routing_weights=routing_weights)
            expand_conv_fn = functools.partial(self._expand_conv, routing_weights=routing_weights)
            depthwise_conv_fn = functools.partial(self._depthwise_conv, routing_weights=routing_weights)
            project_conv_fn = functools.partial(self._project_conv, routing_weights=routing_weights)

        # creates conv 2x2 kernel
        if self._block_args.super_pixel == 1:
            with tf.variable_scope('super_pixel'):
                x = self._relu_fn(self._bnsp(self._superpixel(x), training=training))
            logging.info('Block start with SuperPixel: %s shape: %s', x.name, x.shape)
...
class Model(tf.keras.Model):
    ...
    def _build(self):
        ...
        if self._global_params.num_classes:
            self._fc = tf.layers.Dense(self._global_params.num_classes, kernel_initializer=dense_kernel_initializer)
        else:
            self._fc = None
    ...
    def call(self, inputs, training=True, features_only=None, pooled_features_only=False):
        """Implementation of call().
        ...
        if not features_only:
            ...
            with tf.variable_scope('head'):
                ...
                if self._global_params.local_pooling:
                    ...
                    kernel_size = [1, shape[self._spatial_dims[0]], shape[self._spatial_dims[1]], 1]
